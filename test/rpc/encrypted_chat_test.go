...
func sendAndVerifyMessage(t *testing.T, sender *app.RpcClient, receiver *app.RpcClient, message string, expectedType chat.EncryptionType) {
	err := sender.ChatClient.SendMessage(context.Background(), receiver.CurrentUserID, 0, []byte(message))
	if err != nil {
	...
	case msg := <-receiver.ChatClient.MessageChannel:
		...
		decrypted, err := receiver.ChatClient.DecryptMessage(context.Background(), msg)
		if err != nil {
...
func TestOneUserSendingMessagesPreKeyOnly(t *testing.T) {
	...
	messageFromUser1 := "Hello, this is an encrypted message from User1 to User2"
...
func TestSenderAndReceiverSaveEncryptedChatHistory(t *testing.T) {
	...
	message := "Encrypted message from User1 to User2"
	...
	if err != nil || chatHistory1[0].Message != message {
	...
	if err != nil || chatHistory2[0].Message != message {
